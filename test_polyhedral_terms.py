...
from PolyhedralContracts import Var, Term, TermList
...
class TestTerms(unittest.TestCase):
    ...
    def test_one_term_simpl(self):
        ...
        terml_a.abduceWithHelpers(terml_b, {Var('o')})
        logging.info(terml_a)
        ref = TermList([Term(variables={Var('i'):1}, constant=-1)])
        self.assertEqual(terml_a, ref)
    ...
    def test_two_term_simpl(self):
        ...
        logging.info("*** Begin test")
        terml_a = TermList([Term(variables={Var('o'):1}, constant=-1)])
        terml_b = TermList([Term(variables={Var('i'):-1,Var('o'):1}, constant=0), Term(variables={Var('o_a'):-1,Var('o'):1}, constant=0)])
        logging.info(terml_a)
        logging.info(terml_b)
        terml_a.abduceWithHelpers(terml_b, {Var('o')})
        logging.info(terml_a)
