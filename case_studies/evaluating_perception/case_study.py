...
def initialize_ped(vmax, MAX_V, true_env_type):
    ...
    for xcar_ii in range(xcar_stop + 1, Ncar + 1):
        good, bad, gst, bst = get_formula_states(
            xcar_ii
        )  # We only want the bad states; ignore the good states output here
        bad_states |= bst
        phi2 = phi2 + "|" + bad
    phi2 = phi2 + ")"
...
def gen_points(Ncar, Vlow, Vhigh, xped, bad_states, good_state, formula, vmax):
    ...
    for tp_ped in tp_range:
        for i in range(Nrand):
            ...
            MC, prob = get_states_and_controllers(
                Cped, Mped, start_state, good_state, bad_states, K, K_backup, true_env_type="ped"
            )
...
def gen_points_obj(Ncar, Vlow, Vhigh, xped, bad_states, good_state, formula, vmax):
    ...
    for tp_obj in tp_range:
        for i in range(Nrand):
            ...
            MC, prob = get_states_and_controllers(
                Cobj, Mobj, start_state, good_state, bad_states, K, K_backup, true_env_type="obj"
            )
            # prob = MC.prob_TL(formula)
...
def gen_points_emp(Ncar, Vlow, Vhigh, xped, bad_states, good_state, formula, vmax):
    ...
    for tp_emp in tp_range:
        for i in range(Nrand):
            ...
            MC, prob = get_states_and_controllers(
                Cemp, Memp, start_state, good_state, bad_states, K, K_backup, true_env_type="empty"
            )
            # prob = MC.prob_TL(formula)
