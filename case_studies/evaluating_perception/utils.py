...
class MarkovChain:
    def __init__(self, S, O, state_to_S):
        ...
        self.formula = []
        self.K_int_state_map = (
            dict()
        )  # Nested dictionary mapping internal states to concrete states of the controller instantiation
        self.K_int_state_map_inv = (
            dict()
        )  # Nested dictionary mapping concrete states of controller instance into internal states
    ...
    def compute_next_state(
        self, obs, env_st, init_st
    ):  # The next state computed using the observation from the current state
        Ki = self.K[obs]  # The controller object
        Ki_strategy = self.K_strategy[obs]
        K_instant, sinit_adj, flg = self.adjust_state(
            Ki, Ki_strategy, obs, init_st
        )  # If flg = 1, use sinit_adj as given by backup controller
        if flg == 0:
    ...
    def adjust_state(self, Ki, Ki_strategy, obs, sinit_st):
        ...
        if sinit_st not in self.K_int_state_map_inv[Ki][inp_st].keys():
            ...
            test_list = [
                p for p in poss_st if (p[0] == sinit_st[0] and p[1] == 1)
            ]  # If sinit_st is at zero velocity, car should not remain stuck at 0 velocity.
            flg = 1
            K_instant = None
            if test_list:
    ...
    def get_env_state(self, obs):
        env_st = [
            1
        ]  # For static environment, env state is the same. Should modify this function for reactive environments
        if obs == self.true_env_type:
            env_st = [int(self.true_env)]
        return env_st

    # Constructing the Markov chain
    def construct_markov_chain(
        self,
    ):  # Construct probabilities and transitions in the markov chain given the controller and confusion matrix
