...
def test_basic_fileio():
    ...
    for machine in [True, False]:
        write_contracts_to_file(c_list, c_names, file_name="test_fileio.json", machine_representation=machine)
        c_list_read, c_names_read = read_contracts_from_file("test_fileio.json")
        for i in range(len(c_list)):
            assert c_list_read[i] == c_list[i]
        assert c_names_read == c_names

    c_list, c_names = read_contracts_from_file(io_poly_compound)
    write_contracts_to_file(c_list, c_names, file_name="test_fileio.json", machine_representation=False)
    c_list_read, c_names_read = read_contracts_from_file("test_fileio.json")
    for i in range(len(c_list)):
        assert c_list_read[i] == c_list[i]
    assert c_names_read == c_names
