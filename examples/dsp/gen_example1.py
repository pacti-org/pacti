...
def form_contract(in_port1, in_port2, out_port):
    ret_contract = {}
    ...
    a_bound = get_assumption_bound(in_port_1=in_port1, in_port_2=in_port2, out_port=out_port)
    ret_contract["assumptions"] =  [{"coefficients":{f"{in_port1.name}_a":1, f"{in_port2.name}_a":1},
                                   "constant":a_bound}]
    error_bound = get_guarantee_bound_add(in_port_1=in_port1, in_port_2=in_port2, out_port=out_port)
...
if __name__ == "__main__":
    ...
    c1 = form_contract(in_port1=p1, in_port2=p2, out_port=p3)
    contracts.append(c1)
    p4 = PortWordLength(n=7, p=3, name = "p4")
    p5 = PortWordLength(n=6, p=3, name = "p5")
    c2 = form_contract(in_port1=p3, in_port2=p4, out_port=p5)
    contracts.append(c2)
